/*
  ESP32 NODE PHONG KHACH
  - Gui du lieu MQTT ve Raspberry Pi local
  - Canh bao tai cho bang LED + coi
  - Mat WiFi / mat MQTT van canh bao tai cho
*/
...
//================ NODE PHONG KHACH =================
const char* nodeName   = "phongkhach";
const char* clientHead = "ESP32_PHONGKHACH_";
const char* baseTopic  = "nha/phongkhach";
...
void setup() {
  ...
  Serial.println();
  Serial.println("=================================");
  Serial.println("ESP32 NODE PHONG KHACH KHOI DONG");
...
//==================================================
void loop() {
  // 1. Luôn đọc cảm biến và cảnh báo tại chỗ trước
  ...
  // 2. WiFi xử lý sau, không làm mất cảnh báo tại chỗ
  checkWiFi();

  // 3. MQTT chỉ xử lý khi có WiFi
  if (WiFi.status() == WL_CONNECTED) {
    if (!client.connected()) {
      unsigned long now = millis();
  ...
  // 4. Gửi dữ liệu nếu MQTT đang kết nối
